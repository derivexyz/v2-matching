...
contract LyraForkAddAssetsTest is ForkBase {
  ...
  function testFork() external checkFork {
    ...
    (,,IStandardManager.OracleContingencyParams memory oracleContingencyParams,
      IStandardManager.BaseMarginParams memory baseMarginParams) = Config.getSRMParams("OP");
    ...
    for (uint i = 0; i < markets.length; i++) {
      ...
      _call(
        ...
        abi.encodeWithSelector(
          ...
          _getV2CoreContract(markets[i], "base"),
          marketId,
          IStandardManager.AssetType.Base
        )
      );
      ...
      _call(
        address(srm),
        abi.encodeWithSelector(
          srm.setBaseAssetMarginFactor.selector,
          marketId,
          baseMarginParams.marginFactor,
          baseMarginParams.IMScale
        )
      );
