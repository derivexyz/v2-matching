...
import {IPMRMLib} from "v2-core/src/interfaces/IPMRMLib.sol";
import {IPMRM} from "v2-core/src/interfaces/IPMRMLib.sol";
...
library Config {
  ...
  function getSRMParams(string memory market) public pure returns (
    ...
    } else if (keccak256(abi.encodePacked(market)) == keccak256(abi.encodePacked("DRV"))) {
      baseMarginParams = IStandardManager.BaseMarginParams({
        marginFactor: 0,
        IMScale: 0
      });
    ...
    } else if (keccak256(abi.encodePacked(market)) == keccak256(abi.encodePacked("SolvBTCBBN"))) {
      baseMarginParams = IStandardManager.BaseMarginParams({
        marginFactor: 0.65e18,
        IMScale: 0.93e18
      });
    } else if (keccak256(abi.encodePacked(market)) == keccak256(abi.encodePacked("deUSD"))) {
      baseMarginParams = IStandardManager.BaseMarginParams({
        marginFactor: 0.85e18,
        IMScale: 0.93e18
      });
